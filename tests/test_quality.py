...
class TestQualityChecker:
    def test_passing_rule(self, duck_conn):
        duck_conn.execute(
            "CREATE TABLE test AS SELECT 1 AS id, 'Alice' AS name"
        )
        rule = QualityRule(
            name="always_true",
    ...
    def test_failing_blocking_rule(self, duck_conn):
        duck_conn.execute(
            "CREATE TABLE test AS SELECT NULL AS name"
        )
        rule = no_nulls("name")
        checker = QualityChecker(duck_conn)
        result = checker.check("test", [rule])
    ...
    def test_min_row_count_builtin(self, duck_conn):
        duck_conn.execute(
            "CREATE TABLE test AS SELECT range AS id FROM range(5)"
        )
        checker = QualityChecker(duck_conn)
    ...
    def test_column_positive_builtin(self, duck_conn):
        duck_conn.execute(
            "CREATE TABLE test AS SELECT 5 AS amount UNION ALL SELECT 10"
        )
        checker = QualityChecker(duck_conn)
    ...
    def test_column_positive_fails_on_zero(self, duck_conn):
        duck_conn.execute(
            "CREATE TABLE test AS SELECT 0 AS amount"
        )
        checker = QualityChecker(duck_conn)
        result = checker.check("test", [column_positive("amount")])
        assert result["passed"] is False
